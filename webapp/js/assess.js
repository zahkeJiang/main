	...
	$.post("getComment", {
		...
		if (datas.status == 0) {
			...
			$.each(villa_assess_list, function(commentIndex, comment) {
				...
				//将用户数据循环添加
				html += '<div class="assessBox"><div class="username">' + nickname + '</div><div class="assess-header"><div class="assess-header-icon"><img src="' + comment.headimageurl + '" height="56px"  width="56px"></div><div class="assess-header-content"><div class="assess-header-star">' + star + '</div><div class="assess-header-time">' + commentTime + '</div><p class="assess-header-text"><span>' + comment.content + '</span></p></div></div>' + pictureUrl + '</div>'
			});
			$(".main").html(html);
