  ...
  $.post("getAwards", {
    ...
    if (datas.status == 0) {
      ...
      $.each(awardsList, function(index, comment) {
        ...
        awardsListHtml += '<div class="duihuan-content" awardId="' + comment.awardId + '">' + '<img src="' + comment.image + '" height="80px" width="80px">' + duihuanStatus + '<div class="duihuan-content-text">' + '<p class="name">' + comment.awardName + '</p>' + '<div class="duihuan-money">'+ '<img src="./images/spreadWChat/spreadGold.png" height="16px">' + '<span>' + comment.coin + '</span>'  + '<img src="./images/spreadWChat/normalGold.png" height="16px">' + '<span>' + comment.generalCoin + '</span>' + '</div>' + '<p class="shengyu">剩余：' + comment.amount + '</p>' + '</div>' + '</div>'
      });
    }
